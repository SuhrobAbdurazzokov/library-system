...
let TokenService = class TokenService {
    ...
    async refreshToken(payload) {
        return this.jwt.signAsync(payload, {
            secret: config_1.config.REFRESH_TOKEN_SECRET_KEY,
            expiresIn: config_1.config.REFRESH_TOKEN_TIME,
        });
    }
    async accessToken(payload) {
        return this.jwt.signAsync(payload, {
            secret: config_1.config.ACCESS_TOKEN_SECRET_KEY,
            expiresIn: config_1.config.ACCESS_TOKEN_TIME,
        });
    }
    async writeToCookie(res, key, value, time) {
